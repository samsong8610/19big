...
public class WeChatOAuth2Template implements OAuth2Operations {
    ...
    /**
     * Set authentication url to enable login by scanning a QRCode.
     * @param qrConnectUrl the url provided by WeChat to login by scanning a QRCode.
     */
    ...
    public String buildAuthorizeUrl(OAuth2Parameters parameters) {
        String authUrl = selectAuthorizeUrl(parameters);
        return buildAuthUrl(authUrl, GrantType.AUTHORIZATION_CODE, parameters);
    }

    @Override
    public String buildAuthorizeUrl(GrantType grantType, OAuth2Parameters parameters) {
        String authUrl = selectAuthorizeUrl(parameters);
        return buildAuthUrl(authUrl, grantType, parameters);
    }
    ...
    private String selectAuthorizeUrl(OAuth2Parameters parameters) {
        String scope = parameters.getScope();
        String authUrl = authorizeUrl;
        String qrSuffix = "_qrconnect";
        if (scope != null && scope.endsWith(qrSuffix)) {
            String originScope = scope.substring(0, scope.length() - qrSuffix.length());
            logger.info("Using qrconnect authorize url with scope " + originScope);
            parameters.setScope(originScope);
            authUrl = this.qrConnectUrl + authorizeUrl.substring(authorizeUrl.indexOf('?'));
        }
        return authUrl;
    }
}
